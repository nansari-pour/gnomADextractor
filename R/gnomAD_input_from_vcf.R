...
for (i in 1:22){
  ...
  write.table(snv_chr[,c("CHROM","POS","POS","REF","ALT")],paste0(gsub(".vcf","",input_vcf),"_chr",i,"_gnomAD_input.txt"),col.names = F,row.names = F,quote = F,sep=" ")
  print(i)
}
}
...
# Run the function
gnomAD_input_from_vcf(input_vcf = vcf_file)
